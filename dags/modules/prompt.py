...
def build_prompt(title, summary, content, url):
    # If content is very long, consider truncating or summarizing here.
    user_content = f"Title: {title}\n"
    if summary:
        user_content += f"Summary: {summary}\n"
    else:
        user_content += "Summary: None\n"
    user_content += f"Content: {content}\nURL: {url}\n\n" \
                    f"Extract the fields (report_date, seizure_date, location, items_seized, arrest_count, amount_approximate, comment, url) in JSON."
    messages = [
        system_message,
        {"role": "user", "content": example_article},
        {"role": "assistant", "content": json.dumps(example_output)},  # the example output as JSON string
